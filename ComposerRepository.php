...
class ComposerRepository extends ArrayRepository implements ConfigurableRepositoryInterface
{
    ...
    protected function loadRootServerFile()
    {
        ...
        if (!empty($data['mirrors'])) {
            foreach ($data['mirrors'] as $mirror) {
                if (!empty($mirror['git-url'])) {
                    $this->sourceMirrors['git'][] = array(
                        'url' => $mirror['git-url'],
                        'preferred' => !empty($mirror['preferred'])
                    );
                }
                if (!empty($mirror['hg-url'])) {
                    $this->sourceMirrors['hg'][] = array(
                        'url' => $mirror['hg-url'],
                        'preferred' => !empty($mirror['preferred'])
                    );
                }
        ...
        if (preg_match('{^https?://repo\.packagist\.org/?$}i',
                $this->url) && !empty($this->repoConfig['force-lazy-providers'])) {
    ...
    protected function fetchFile($filename, $cacheKey = null, $sha256 = null, $storeLastModifiedTime = false)
    {
        ...
        while ($retries--) {
            try {
                $preFileDownloadEvent = new PreFileDownloadEvent(PluginEvents::PRE_FILE_DOWNLOAD, $this->rfs,
                    $filename);
    ...
    public function whatProvides(Pool $pool, $name, $bypassFilters = false)
    {
        ...
        if (!$this->hasPartialPackages || !isset($this->partialPackagesByName[$name])) {
            ...
            if (preg_match(PlatformRepository::PLATFORM_PACKAGE_REGEX,
                    $name) || '__root__' === $name || 'composer-plugin-api' === $name) {
                return array();
            }
        ...
        foreach ($packages['packages'] as $versions) {
            foreach ($versions as $version) {
                ...
                if (isset($this->providersByUid[$version['uid']])) {
                    ...
                    if (!$bypassFilters && !$pool->isPackageAcceptable(strtolower($version['name']),
                    ...
                    if (isset($rootAliasData)) {
                        $alias = $this->createAliasPackage($package, $rootAliasData['alias_normalized'],
                            $rootAliasData['alias']);
    ...
    protected function createPackage(array $data, $class = 'Composer\Package\CompletePackage')
    {
        try {
            ...
            throw new RuntimeException('Could not load package ' . (isset($data['name']) ? $data['name'] : json_encode($data)) . ' in ' . $this->url . ': [' . get_class($e) . '] ' . $e->getMessage(),
                0, $e);
        }
    }
    ...
    protected function initialize()
    {
        ...
        foreach ($repoData as $package) {
            $package = $this->__parseVersionConstraint($package, $versionParser);
     ...
     * @return mixed
     */
    protected function __parseVersionConstraint($package, VersionParser $versionParser)
    {
        if (isset($package['require'])) {
            $req = [];
            foreach ($package['require'] as $packName => $const) {
                try {
                    if ($versionParser->parseConstraints($const)) {
                        $req[$packName] = $const;
                    }
